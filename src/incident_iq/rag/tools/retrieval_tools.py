...
from ...database.models import RBACModel
...
def permission_check_tool(user_id: str, chunk_ids: str, db_service) -> str:
    ...
    NO RAW SQL - Uses RBACModel methods
    ...
    try:
        ...
        # Get user's CDR codes using model method - NO RAW SQL
        user_roles_list = rbac_model.get_user_roles(user_id)
        user_roles = set(role['role_code'] for role in user_roles_list) if user_roles_list else set()
        ...
        for chunk_id in chunk_id_list:
            ...
            # Get required permissions using model method - NO RAW SQL
            required_roles_list = rbac_model.get_document_permissions(doc_id)
            required_roles = set(perm['required_code'] for perm in required_roles_list) if required_roles_list else set()
