...
class LLMService:
    ...
    def _initialize_embeddings(self):
        ...
        print(f"[LLMService] Initializing embeddings with provider: {default_provider}")
        print(f"[LLMService] Embedding config: {embed_config}")
        ...
        try:
            if default_provider == 'openai':
                ...
                # Unknown provider, try sentence transformers as fallback
                from langchain_huggingface import HuggingFaceEmbeddings
                print(f"[WARNING] Unknown embedding provider: {default_provider}, using sentence_transformers")
                return HuggingFaceEmbeddings(model_name='all-MiniLM-L6-v2')
        except Exception as e:
            print(f"[ERROR] Failed to initialize embeddings for {default_provider}: {e}")
            # Fallback to sentence transformers
            try:
                from langchain_huggingface import HuggingFaceEmbeddings
                return HuggingFaceEmbeddings(model_name='all-MiniLM-L6-v2')
            except Exception as e2:
                print(f"[ERROR] Fallback embeddings also failed: {e2}")
                return None
